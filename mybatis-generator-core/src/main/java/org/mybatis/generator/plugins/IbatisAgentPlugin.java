 ...
 * @date 2017年8月28日
 */
public class IbatisAgentPlugin extends PluginAdapter {
	...
	public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable) {
		...
		interfaceType = new FullyQualifiedJavaType(servicePack + "." + tableName + "Agent");

		daoType = new FullyQualifiedJavaType(introspectedTable.getDAOInterfaceType());

		// logger.info(toLowerCase(daoType.getShortName()));
		serviceType = new FullyQualifiedJavaType(serviceImplPack + "." + tableName + "AgentImpl");
